...
- **The license key is NEVER committed.** It is a free hobby key registered for the dashboard's production HTTPS tailnet domain. It is injected at runtime via the `sketch:init` postMessage config (§5.3). **Concrete channel (Q4, built):** the orchestrator reads env var **`TLDRAW_LICENSE_KEY`** into `RouteContext.tldrawLicenseKey` and exposes it via **`GET /api/sketch/config`** (mirroring the existing `/api/voice/status` config-delivery pattern); the dashboard fetches it once at boot (`loadSketchConfig`), stashes it on `state.sketchLicenseKey`, and `chat.ts` passes it into `mountSketchCanvas(..., { licenseKey })` → `sketch:init` → `<Tldraw licenseKey={key}>`. Rotating the key changes one env var, not the bundle. See §9.5 for the full channel + tests.
- **Dev mode needs NO key.** When the browser reaches the dashboard over HTTP or via `localhost`, the SDK is in development mode → free, no key, no watermark, no violation. With `TLDRAW_LICENSE_KEY` unset (the current state), `GET /api/sketch/config` returns `{}`, `state.sketchLicenseKey` stays null, and `sketch:init` omits `licenseKey` — byte-for-byte the no-key mount.
- **Production mode (HTTPS non-localhost) MUST be tested, not assumed-localhost.** Q4 exercises the production path via the repo's existing e2e harness (`src/test/ui/`, `node --test` — there is no Playwright in this repo): the e2e (`src/test/ui/sketch.test.ts`) drives the real dashboard pipeline with the tldraw iframe/postMessage boundary MOCKED, mounts the canvas with a license key, and asserts (a) the key rides `sketch:init` after the handshake, and (b) the production-license path forwards the key end-to-end. Unit tests assert the server emits the key only when env-set (`routes.sketch-config.test.ts`) and the dashboard stashes only a non-empty key (`state.sketch-config.test.ts`). The committed source/bundle contains no key (it lives only in the env var). We do not ship trusting that everyone reaches it via `localhost`.
...
- **Export scale cap + raster ceiling INSIDE the iframe:** the `scale` in `sketch:export-request` is **clamped** to `[MIN_SCALE, MAX_SCALE]` (= `[0.25, 2]`). **Before** calling `editor.toImage`, the iframe computes the would-be raster dimensions (canvas bounds × clamped scale) and enforces a **concrete raster ceiling**. **Q3 implemented value (`src/shared/sketch-raster.ts`):** the ceiling is THREE caps applied together — `MAX_SCALE = 2`, a **per-edge cap `MAX_EDGE_PX = 2048`** (each side ≤ 2048 px), AND a **total-megapixel cap `MAX_RASTER_PX = 32_000_000`** (32 MP). The per-edge cap closes the long-thin-canvas DoS the megapixel cap alone misses (e.g. 50000×100 is only 5 MP yet 50000 px wide). The pure `clampExportDimensions`/`decideExport` gate **down-scales** a large-but-legitimate canvas so it DEGRADES SAFELY to a smaller-but-valid image instead of failing, while an empty/no-bounds canvas returns `sketch:export-response { ok:false, error:'empty canvas' }` WITHOUT reaching `toImage`. **We do NOT rely on the 100 MB `FILE_MAX_BYTES` upload backstop** (`routes.ts`) to catch this — that is a last-resort net far above a sane image, and a 32 MP PNG can be well under 100 MB while still being an OOM/DoS risk to render. The ceiling lives at the rasterization point.
...
- The key is injected via `sketch:init` (§1.0), never committed. **Concrete channel (Q4, built):** the orchestrator reads the env var **`TLDRAW_LICENSE_KEY`** into `RouteContext.tldrawLicenseKey` (`src/orchestrator/main.ts`) and surfaces it through a new endpoint **`GET /api/sketch/config`** (`src/orchestrator/routes.ts`), which mirrors the existing `/api/voice/status` config-delivery pattern. When the env var is SET, the response body is `{ "licenseKey": "<key>" }`; when UNSET, the body is `{}` — the key is **omitted from the wire entirely**, not sent as null/empty. The dashboard fetches this once at boot (`loadSketchConfig` in `src/dashboard/state.ts`), stashes a non-empty key on `state.sketchLicenseKey`, and `chat.ts` forwards it into `mountSketchCanvas(..., { licenseKey })` → `sketch:init` → `<Tldraw licenseKey=…>`. Q4 asserts: the endpoint emits the key only when set (`routes.sketch-config.test.ts`), the dashboard wiring stashes only a non-empty key (`state.sketch-config.test.ts`), and the e2e proves the key rides `sketch:init` on the production path (`src/test/ui/sketch.test.ts`). The key is the ONLY config value that crosses to the iframe; the config endpoint does not expose `ORCHESTRATOR_SECRET` or any other field. Rotating the key changes the env var only, not the bundle.
...
**Scope:** `dirty` tracking (iframe posts `sketch:dirty` on store change, debounced) driving ONLY the `· edited` marker (Send is always enabled, §1.1a); Greenroom chrome (eyebrow, `· edited`, Send clay-primary, Reset); Send flow — parent posts `sketch:export-request` (clamped scale) → receives `sketch:export-response` (PNG dataURL) → converts to a `File` → `POST /api/files` for the **PNG (primary)** AND a second `POST /api/files` for the **DSL sidecar** → `POST /api/dashboard/send` with BOTH `fileId`s (reusing the `uploadFileToStorage` shape from `chat.ts`); `sketch:reset` discards staged edits.

> **Q3 build note — EDITED-DSL sidecar (operator decision, supersedes §1.1b / §14 Q1):** the operator decided the sidecar carries the **EDITED** source (so the agent receives re-editable material reflecting the operator's edits), not the original DSL. Rather than invert the §7 DSL translation (the hard path §14 Q1 defers), the iframe emits the edited canvas as a **re-editable tldraw snapshot** (`editor.getSnapshot()`, JSON-serialized) inside the `sketch:export-response`'s new optional `editedDsl` field; the parent uploads it as `sketch.tldr.json`. The snapshot is lossless and reloadable via `loadSnapshot`. When the iframe cannot serialize the snapshot (older frame / serialization error), the parent **falls back to the original validated DSL** as the sidecar, so Send never blocks on the snapshot. The PNG (`sketch.png`) remains the primary deliverable. The pure Send orchestration is `src/dashboard/sketch-send.ts`; the DOM-free dirty/requestId/reset state machine is `src/dashboard/sketch-send-controller.ts`. This required an iframe-adapter change → the vendored bundle was rebuilt (new sha256 in `VENDOR.md`).
**Checks:**
...
**Checks:**
- **e2e (mock), BUILT** — there is NO Playwright in this repo; the e2e uses the existing `node --test` UI harness (`src/test/runner.ts` + `src/test/mock-server.ts`). `src/test/ui/sketch.test.ts` drives the REAL dashboard pipeline with the tldraw iframe/postMessage boundary MOCKED (a fake iframe window answers the protocol: `sketch:init` → `sketch:ready`, export-request → stub PNG + edited sidecar): sketch message → static SVG preview (`renderBodyWithSketches`) → "Open canvas" mounts the (mocked) iframe (`mountSketchCanvas`) → handshake (`sketch:init` carries the nonce + license key, then `sketch:load`) → staged-edit dirty marker (`sketch:dirty` → `· edited` visible) → Send → export-request → dual upload (PNG primary + sidecar) through `POST /api/files` → `POST /api/dashboard/send` with BOTH fileIds, targeted at the source agent. Asserts on the mock request log + the send body. Green, non-flaky (promise-settle + poll-until, no fixed sleeps). *(`src/test/ui/sketch.test.ts`)*
- **Production-mode license path, BUILT** — the same e2e mounts the canvas with a `licenseKey` and asserts it rides `sketch:init` after the handshake (the production-license forward). Server-side: `routes.sketch-config.test.ts` asserts `GET /api/sketch/config` emits the key ONLY when `TLDRAW_LICENSE_KEY` is set (omits it otherwise) and never leaks `ORCHESTRATOR_SECRET`. Dashboard-side: `state.sketch-config.test.ts` asserts `loadSketchConfig` stashes only a non-empty key. No committed source/bundle contains the key (it lives only in the env var). *(`src/test/ui/sketch.test.ts`, `routes.sketch-config.test.ts`, `state.sketch-config.test.ts`)*
