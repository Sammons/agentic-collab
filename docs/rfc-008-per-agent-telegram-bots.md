...
- On boot (after persona sync). **IMPLEMENTED (PR-C) — coexistence, NOT replacement:** the legacy `listDestinations()` startup loop is KEPT (the `destinations` table stays for global/non-agent bots per Non-Goals); reconcile runs *after* it and seeds its dedup set with the destination tokens, so a per-agent bot never becomes a second consumer of a token a destination already polls. This supersedes the earlier "replaces the destinations startup loop" plan (deleting it would stop global destinations).
- On persona reload / `POST /api/personas/reload` / sync — re-reconcile (start new, stop removed, restart on token/chatId/routing change).
- On agent delete / suspend. **IMPLEMENTED (PR-C):** stop inbound when the agent is removed (and cascade-delete its encrypted token row, since the token is AAD-bound to the agent name); keep polling while suspended so messages queue.
...
- **PR-C (resolution + reconcile + inbound) — MERGED:** token resolution = **decrypt-at-reconcile** from the AES-256-GCM SQLite store (`getTelegramToken`, AAD = agent name; the env/destinations/age-secrets resolution was superseded by the §2 encrypted-store decision delivered in PR-Bsec); `reconcileTelegramBots` **owns the per-agent poll set and COEXISTS with the legacy destinations loop** (kept; seeds its dedup set with destination tokens — see §6), deduping on **resolved token**, not bot name (two names → same token = 409); per-agent inbound routing (self/prefix/passthrough); `void`-agent policy = **reply-on-offline** ("`<agent>` is offline", operator decision #3, NOT auto-spawn); cascade-delete the token row on agent delete; quiet the transient restart 409 (§183); wire boot + persona-reload reconcile.
...
- **Dedupe on resolved token, not bot name** (two names can resolve to one token → 409). **As built (PR-C):** reconcile does NOT delete the legacy `main.ts` startup loop — that loop is kept for global/non-agent destinations (Non-Goals). Instead reconcile **seeds its dedup set with the legacy loop's destination tokens** and runs after it, so a per-agent bot never becomes a second consumer of a destination's token (warn + skip). Same 409 protection, without dropping global destinations.
- **Restart-race 409:** abort cancels the in-flight long-poll (good), but Telegram's server-side consumer state lingers briefly → transient 409 on rotation/reload; current retry self-heals but log-spams. **Resolved (PR-C):** the poll loop special-cases `409` to a single `console.warn` ("transient consumer conflict") instead of `console.error`, so a rotation/reload no longer spams the error log.
