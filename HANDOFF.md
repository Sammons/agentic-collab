...
Messages are enqueued in `pending_messages` table. The health monitor delivers one message per poll cycle when an agent is idle. Messages are pasted into the tmux session via the proxy. Format: `[from: <source>: reply with collab send <source> --topic <topic>]: '<message>'`.
