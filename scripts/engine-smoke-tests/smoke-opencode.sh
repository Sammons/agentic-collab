#!/usr/bin/env bash
# Smoke tests for OpenCode CLI — validates adapter assumptions match reality.
#
# DRIFT STATUS (2026-03-08):
# OpenCode v1.2.10 changed behavior:
...
#   - Adapter still assumes `opencode run` starts an interactive REPL with `> ` prompt
# These tests validate what the CLI *actually does* to catch this drift.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/lib.sh"

require_engine opencode || exit 0

echo ""
echo "══════════════════════════════════════"
echo "  OpenCode CLI Smoke Tests"
echo "══════════════════════════════════════"

# ── Test 1: TUI mode spawn (default command) ──
...
test_tui_spawn() {
  ...
  # Default TUI mode: just `opencode` (no `run` subcommand)
  paste_and_enter "$s" "opencode"

  # TUI shows "Ask anything..." placeholder and OPENCODE banner
  assert_pattern "$s" "tui: OpenCode TUI starts" 'Ask anything|OPENCODE|opencode'
...
# ── Test 2: `opencode run` without message (drift detection) ──
...
test_run_requires_message() {
  ...
  # opencode run (no message) — should error in current version
  paste_and_enter "$s" "opencode run"

  # Current behavior: "You must provide a message or a command"
  # If this starts passing (showing a prompt), the adapter can use `opencode run` again
...
# ── Test 3: Paste delivery in TUI mode ──
...
test_paste_delivery() {
  ...
  if ! wait_for_pattern "$s" 'Ask anything|OPENCODE|opencode' "$TIMEOUT"; then
    fail "paste: OpenCode TUI never showed"
    kill_session "$s"
    return
  fi

  # In TUI mode, text is typed into the prompt area
...
test_exit_tui() {
  ...
  if ! wait_for_pattern "$s" 'Ask anything|OPENCODE|opencode' "$TIMEOUT"; then
    fail "exit: OpenCode TUI never showed"
    kill_session "$s"
    return
  fi
  ...
  if wait_for_pattern "$s" '[\$%#] ?$' "$TIMEOUT"; then
    ...
    # TUI might need Ctrl-C or q to exit
...
# ── Test 5: Resume with -c (continue last) ──
...
test_resume_continue() {
  ...
  # Try opencode run -c — should either resume or give graceful error
...
test_resume_session() {
  ...
  # Try resuming a nonexistent session — should fail gracefully
